angular.module('ui.multiselect', [
  ...
  .directive('multiselect', ['$parse', '$document', '$compile', '$interpolate', 'optionParser', '$filter',

    function ($parse, $document, $compile, $interpolate, optionParser, $filter) {
      return {
        restrict: 'E',
        require: 'ngModel',
        link: function (originalScope, element, attrs, modelCtrl) {
          ...
          var exp = attrs.options,
            ...
            displayFilter = attrs.displayfilter;
              ...
              if (isMultiple) {
                  if (attrs.msSelected) {
                	  ...
                	  if(!displayFilter) {
                		  ...
                		  scope.header = '';
                		  angular.forEach(modelCtrl.$modelValue, function (modVal, idx) {
                			  if(scope.header == '') {
                				  scope.header = $filter(displayFilter)(modVal.value);
                			  } else {
                				  scope.header = scope.header + ', ' + $filter(displayFilter)(modVal.value);
                			  }
                		  });
                	  }
                  }
